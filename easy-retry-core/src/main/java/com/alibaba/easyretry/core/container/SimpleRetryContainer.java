...
public class SimpleRetryContainer implements RetryContainer {
    ...
    public SimpleRetryContainer(RetryConfiguration retryConfiguration, String namespace, RetryExecutor retryExecutor) {
    ...
    public void start() {
        ...
        ThreadPoolExecutor retryExecutor = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS,
            new LinkedBlockingQueue<>(), r -> new Thread(r, "retryExecutor-Thread"));
        retryExecutor.execute(new TaskConsumer(queue));
        ThreadPoolExecutor retrySelector = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS,
            new LinkedBlockingQueue<>(), r -> new Thread(r, "retrySelector-Thread"));
    ...
    public class TaskConsumer implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                try {
                    Thread.sleep(totalTime > MAX_SLEEP_TIME_MILLISECONDS ? MAX_SLEEP_TIME_MILLISECONDS : totalTime);
    ...
    public class TaskProducer implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                doSelect();
                long totalTime = sleepTimes * SLEEP_BASE_TIME_MILLISECONDS + SLEEP_BASE_TIME_MILLISECONDS;
                try {
                    Thread.sleep(totalTime > MAX_SLEEP_TIME_MILLISECONDS ? MAX_SLEEP_TIME_MILLISECONDS : totalTime);
                } catch (InterruptedException e) {
                    log.error("taskConsumer interruptedException error", e);
        ...
        private void doSelect() {
            ...
            List<RetryTask> tasks = retryConfiguration.getRetryTaskAccess().listAvailableTasks(namespace, lastId);
            ...
            for (RetryTask task : tasks) {
                try {
                    lastId = task.getId();
                    RetryContext retryContext = new RetryContextBuilder(retryConfiguration, task).buildArgs()
                    ...
                    log.warn("add retry task to queue. namespace:{}, task:{}", namespace, task.getId());
                } catch (Throwable e) {
                    log.error("add retry task to queue. namespace:{}, task:{}", namespace, task.getId(), e);
